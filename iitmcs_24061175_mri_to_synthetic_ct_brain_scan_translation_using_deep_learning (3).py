...
# Training
model_keras.fit(train_gen, epochs=10, validation_data=val_gen)
...
import torch
import streamlit as st # Import streamlit if needed for this snippet
import numpy as np
from skimage import filters # Import filters

# Load your PyTorch model once (outside main loop!)
# Ensure the model file exists or handle the FileNotFoundError
# model = torch.load("your_ct_model.pth") # This line will cause FileNotFoundError if the file doesn't exist
# model.eval() # This line will cause NameError if model is not loaded

# Assuming 'norm_img' is defined and contains the normalized input image
# Assuming 'seg_model' is defined and contains a segmentation model

# On button click, prepare the MRI slice/tensor and run inference:
# This part looks like it belongs inside a function or block that is triggered by a Streamlit button
# if st.button("Generate CT"): # Example Streamlit button
#     # Assuming norm_img is available here
#     input_tensor = torch.from_numpy(norm_img).unsqueeze(0).unsqueeze(0).float().to(device) # device needs to be defined
#     with torch.no_grad():
#         # model needs to be loaded and available here
#         ct_output = model(input_tensor).cpu().numpy()[0,0,:,:]
#     st.image(ct_output, caption="Predicted CT", use_column_width=True, channels="L")

# Add more buttons: Smoothing, Thresholding, Histogram Equalization
# if st.button("Apply Smoothing"): # Example Streamlit button
#     # Assuming norm_img is available here
#     smoothed = filters.gaussian(norm_img, sigma=1)
#     st.image(smoothed, caption="Smoothed Image", use_column_width=True)

# This loop appears to be for batch mode in a Streamlit app
# uploaded_files = st.file_uploader("Upload MRI images", type=["nii","nii.gz"], accept_multiple_files=True)
# for file in uploaded_files:
#     # Same logic as in single file, but loop for batch
#     # Save, load, process, show results for each file
#     pass # Placeholder for batch processing logic


# This part looks like it belongs within a Streamlit tab or section for segmentation
# tab1, tab2, tab3 = st.tabs(["CT Generation", "Batch Mode", "Segmentation"])
# with tab1:
#     # Single-file CT workflow (code from above)
#     pass
# with tab2:
#     # Loop over uploaded_files list (code from above)
#     pass
# with tab3:
#     # Segmentation workflow
#     # Assuming seg_model is loaded and available
#     # Assuming input_tensor is prepared
#     # mask = seg_model(input_tensor)  # shape: [H, W] # This line was causing the IndentationError
#     # st.image(mask, caption="Segmentation Mask", use_column_width=True)
#     # # Overlay on original
#     # # Assuming norm_img is available
#     # overlay = np.stack([norm_img, mask*255], axis=-1)  # Simple example
#     # st.image(overlay, caption="MRI + Segmentation Overlay", use_column_width=True)
#     pass


# This download button also looks like it belongs in a Streamlit app, likely after CT generation
# st.download_button("Download CT", ct_output.tobytes(), file_name="ct.nii.gz")

# The original code had an indentation error here:
# mask = seg_model(input_tensor)  # shape: [H, W]
# st.image(mask, caption="Segmentation Mask", use_column_width=True)
# # Overlay on original
# overlay = np.stack([norm_img, mask*255], axis=-1)  # Simple example
# st.image(overlay, caption="MRI + Segmentation Overlay", use_column_width=True)
...
# tab1, tab2, tab3 = st.tabs(["CT Generation", "Batch Mode", "Segmentation"])
# with tab1:
#     # Single-file CT workflow
# with tab2:
#     # Loop over uploaded_files list
# with tab3:
#     # Segmentation workflow

# Corrected indentation for the code block that was causing the error
# This block likely belongs inside the 'with tab3:' section or a similar conditional block
# if 'seg_model' in locals() and 'input_tensor' in locals(): # Example condition
#     mask = seg_model(input_tensor)  # shape: [H, W]
#     st.image(mask, caption="Segmentation Mask", use_column_width=True)
#     # Overlay on original
#     if 'norm_img' in locals(): # Check if norm_img is available for overlay
#         overlay = np.stack([norm_img, mask*255], axis=-1)  # Simple example
#         st.image(overlay, caption="MRI + Segmentation Overlay", use_column_width=True)

# The tab definition and usage should wrap the relevant code blocks
# tab1, tab2, tab3 = st.tabs(["CT Generation", "Batch Mode", "Segmentation"])
# with tab1:
#     # CT Generation code
#     pass
...
# This cell seems to be a collection of unrelated code snippets for a Streamlit app.
# It's best to integrate these into the main Streamlit script (app.py) if that's the intention.
# For now, I will just fix the indentation of the original problematic lines.
# However, running these lines here without the rest of the Streamlit app context
# and undefined variables will likely lead to other errors.

# Original problematic lines with corrected indentation (assuming they belong together)
# Note: These lines still depend on 'seg_model', 'input_tensor', and 'norm_img' being defined.
# if 'seg_model' in locals() and 'input_tensor' in locals():
#     mask = seg_model(input_tensor)  # shape: [H, W]
#     st.image(mask, caption="Segmentation Mask", use_column_width=True)
#     # Overlay on original
#     if 'norm_img' in locals():
#         overlay = np.stack([norm_img, mask*255], axis=-1)  # Simple example
#         st.image(overlay, caption="MRI + Segmentation Overlay", use_column_width=True)

# The download button is also likely associated with the CT generation part
# if 'ct_output' in locals():
#      st.download_button("Download CT", ct_output.tobytes(), file_name="ct.nii.gz")

# The tab definitions should wrap the entire content of the app
# tab1, tab2, tab3 = st.tabs(["CT Generation", "Batch Mode", "Segmentation"])
# with tab1:
#    # CT Generation code here
#    pass
# with tab2:
#    # Batch Mode code here
#    pass
# with tab3:
#    # Segmentation code here
#    pass

# Removing the problematic unindented code from the original cell content
# as it seems to be a collection of scattered snippets.
# The user should integrate these into their Streamlit app.py file.

# The original cell content was a mix of incomplete code blocks and Streamlit calls.
# Simply fixing indentation won't make it runnable in a standard cell.
# It's better to leave this cell as a placeholder or remove it, and guide the user
# to integrate these snippets into their app.py file.

# Since the request is to fix the error in the selected code, and the error was indentation,
# I will provide a version of the code with the indentation fixed, but comment out
# the sections that rely on Streamlit context and undefined variables, explaining
# that this code is likely for the Streamlit app.

# This cell seems to be a collection of code snippets for a Streamlit app.
# It is not intended to be run as a single Colab cell.
# The indentation error is fixed in the commented out code below.
# Please integrate these snippets into your 'app.py' file.
...
# # Load your PyTorch model once (outside main loop!)
# # Ensure the model file exists or handle the FileNotFoundError
# # model = torch.load("your_ct_model.pth")
# # model.eval()
...
# # On button click, prepare the MRI slice/tensor and run inference:
# # if st.button("Generate CT"):
# #     input_tensor = torch.from_numpy(norm_img).unsqueeze(0).unsqueeze(0).float().to(device)
# #     with torch.no_grad():
# #         ct_output = model(input_tensor).cpu().numpy()[0,0,:,:]
# #     st.image(ct_output, caption="Predicted CT", use_column_width=True, channels="L")
# #     st.download_button("Download CT", ct_output.tobytes(), file_name="ct.nii.gz")
...
# As the original cell contained scattered, unindented code,
# I will replace it with a markdown cell explaining its purpose
# and how to use the code snippets in app.py.
